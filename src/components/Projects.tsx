      ...
      <div className="section-container py-0">
        ...
        <ScrollReveal delay={0.2}>
          <h2 className="heading text-center mb-16">
            <AnimatedText text="Recent Work" className="inline-block" />
          </h2>
        </ScrollReveal>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {projects.map((project, index) => <ScrollReveal key={project.title} delay={0.2 + index * 0.1} direction="up">
              <div className="glass hover-scale rounded-xl overflow-hidden h-full flex flex-col">
                ...
                <div className="p-6 flex flex-col flex-grow bg-emerald-950">
                  <h3 className="font-semibold text-xl mb-2">{project.title}</h3>
                  <p className="text-muted-foreground mb-4 flex-grow">{project.description}</p>
                  
                  <div className="flex flex-wrap gap-2 mb-6">
                    {project.technologies.map(tech => <span key={tech} className="px-3 py-1 bg-secondary text-secondary-foreground text-xs rounded-full">
                        {tech}
                      </span>)}
                  </div>
                  
                  <div className="flex gap-4 mt-auto">
                    {project.github && <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm font-medium link">
                        <Github className="mr-1 h-4 w-4" />
                        Source Code
                      </a>}
                    {project.demo && <a href={project.demo} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm font-medium link">
